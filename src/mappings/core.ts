...
import { updatePairDayData, updateTokenDayData, updateStableswapDayData, updatePairHourData } from '../utils/dayUpdates'
import { getEthPriceInUSD, findEthPerToken, getTrackedVolumeUSD, getTrackedLiquidityUSD } from '../utils/pricing'
import {
...
export function handleSync(event: Sync): void {
  ...
  let stableswap = StableswapFactory.load(FACTORY_ADDRESS)!

  // reset factory liquidity by subtracting onluy tarcked liquidity
  stableswap.totalLiquidityETH = stableswap.totalLiquidityETH.minus(pair.trackedReserveETH as BigDecimal)
  ...
  stableswap.totalLiquidityETH = stableswap.totalLiquidityETH.plus(trackedLiquidityETH)
  stableswap.totalLiquidityUSD = stableswap.totalLiquidityETH.times(bundle.ethPrice)
...
export function handleMint(event: Mint): void {
  ...
  stableswap.txCount = stableswap.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
  ...
  let stableswap = StableswapFactory.load(FACTORY_ADDRESS)!
  stableswap.totalVolumeUSD = stableswap.totalVolumeUSD.plus(trackedAmountUSD)
  stableswap.totalVolumeETH = stableswap.totalVolumeETH.plus(trackedAmountETH)
  stableswap.untrackedVolumeUSD = stableswap.untrackedVolumeUSD.plus(derivedAmountUSD)
  stableswap.txCount = stableswap.txCount.plus(ONE_BI)
  ...
  let uniswapDayData = updateStableswapDayData(event)
